...
#define BIG_D2B_B(D, B)		*(unsigned long*)(B) = (unsigned long)(D)
#define BIG_D2B_L(D, B)		*(unsigned long*)(B) = ENDIAN_REVERSE_ULONG(D)
...
int IsLittleEndianV = -1;

int IsLittleEndian2()
{
	if (IsLittleEndianV < 0)
	{
		short a = 0x0001;
...
// o 출력				: 
//*********************************************************************************************************************************
void SHA256_Process(SHA256_INFO *Info, const BYTE *pszMessage, unsigned int uDataLen)
{
...
/*#if defined(LITTLE_ENDIAN)
	...
	for (i = 0; i < SHA256_DIGEST_VALUELEN; i += 4)
	{
		if (IsLittleEndian2())
		{
			BIG_D2B_L((Info->uChainVar)[i / 4], &(pszDigest[i]));
		}
		else
		{
			BIG_D2B_B((Info->uChainVar)[i / 4], &(pszDigest[i]));
		}
...
void SHA256_Encrypt(const BYTE *pszMessage, unsigned int uPlainTextLen, BYTE *pszDigest)
{
	SHA256_INFO info;
...
void SHA256_EncryptLR(const BYTE *pszMessageL, const BYTE *pszMessageR, BYTE *pszDigest)
{
	//char empty[SHA256_DIGEST_BLOCKLEN] = { 0, };

	//if (!memcmp(pszMessageL, empty, SHA256_DIGEST_BLOCKLEN) && !memcmp(pszMessageR, empty, SHA256_DIGEST_BLOCKLEN)) return;

	SHA256_INFO info;

	SHA256_Init(&info);

	SHA256_Process(&info, pszMessageL, SHA256_DIGEST_BLOCKLEN);

	SHA256_Process(&info, pszMessageR, SHA256_DIGEST_BLOCKLEN);
